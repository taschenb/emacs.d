...
While it can be done within Emacs, disabling the bars is rather slow.
Instead, use your ~/Xresources file to speed this up.
...
In order to reload the Xresources after making changes, issue
#+BEGIN_SRC sh
xrdb ~/.Xresources
#+END_SRC

*** Emacs fallback
Even if we do not have a proper Xresources, make sure to still disable
everything
...
#+BEGIN_SRC emacs-lisp
  (use-package doom-themes
    ...
    ;; org-mode: Header same size and without background
...
* Interface-questions
** yes/no to y/n
Just press y/n instead of typing it out
...
Kill running processes without confirmation when exiting Emacs

#+BEGIN_SRC emacs-lisp
(setq confirm-kill-processes nil)
#+END_SRC

** Follow symlinks to file under version control
Do not ask, just follow the symlink

#+BEGIN_SRC emacs-lisp
(setq find-file-visit-truename t vc-follow-symlinks t)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package evil
    ...
    (evil-mode)
    :config
    (evil-define-key 'normal org-mode-map [tab] 'org-cycle) ; cycle
...
** Send buffer
After starting a python process using C-c C-p, send the current buffer
...
#+BEGIN_SRC emacs-lisp
  (defun my-c++-mode-hook ()
    ;; Use a sane style
    (c-set-style "linux")
    ;; Linux style sets it to 8, which can get weird then the tab-width is not
...
#+BEGIN_SRC emacs-lisp
  (use-package eglot
    :init
    ;; Autostart for certain languages
...
** Personal setup
For privacy reasons, the actual mail-setup is not checked in. To
document my usage, the configuration is sketched in notmuch.
  ...
  (use-package notmuch
    ...
    (setq notmuch-saved-searches
          '((:name "inbox" :query "tag:inbox" :key "i")
            ...
            (:name "emacs-list" :query "tag:emacs and tag:list" :key "e")
            (:name "all" :query "*" :key "a")))

    (setq notmuch-saved-searches
          '((:name "inbox unread" :query "tag:inbox and tag:unread" :key "j")
            (:name "inbox" :query "tag:inbox" :key "i")
            (:name "unread" :query "tag:unread" :key "u")
            (:name "gentoo-list" :query "tag:gentoo and tag:list" :key "g")
            (:name "all" :query "*" :key "a"))
    ...
    (define-key notmuch-search-mode-map "S"
      (lambda (&optional beg end)
        ...
        (notmuch-search-tag (list "+spam" "-inbox" "-unread" "-new") beg end))))
#+END_SRC

**** hooks
Notmuch supports hooks to be run on various stages. The hooks
directory goes into .notmuch/hooks inside your mail-directory, where
the xapian directory is also located.

Make sure that the files are executable.

***** pre-new
A simple script so that 'notmuch new' fetches the mails first.
...
***** post-new
This hook does the heavy-lifting for tagging the new messages.
...
#+BEGIN_SRC sh
  ...
  # Inbox
  notmuch tag +inbox -new -- tag:new and to:me@my-domain.com
  notmuch tag +inbox -new -- tag:new and to:me@my-other-domain.com

  # Mailinglists
  notmuch tag +gentoo-dev +gentoo +list -inbox -new -- tag:new and to:gentoo-dev@lists.gentoo.org
  notmuch tag +gentoo-user +gentoo +list -inbox -new -- tag:new and to:gentoo-user@lists.gentoo.org

  # Spam
  notmuch tag +spam -inbox -new -unread -- tag:new and from:*@wolfram.co.uk
  notmuch tag +spam -inbox -new -unread -- tag:new and from:*@wolfram.com
#+END_SRC
* RSS
